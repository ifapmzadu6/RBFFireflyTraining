...
Firefly::Firefly(const Firefly &obj) : dim(obj.dim), dataCount(obj.dataCount), rbfCount(obj.rbfCount), fitness(obj.fitness), weights(obj.weights), spreads(obj.spreads), centerVectors(obj.centerVectors), biases(obj.biases) {
}
...
const double Firefly::norm(const std::vector<double> &a, const std::vector<double> &b) const {
    double d = 0.0;
...
void Firefly::mult(std::vector<std::vector<double>> &Y, const std::vector<std::vector<double>> &A, const std::vector<std::vector<double>> &B) {
    for (auto &vec : Y) {
        for (auto &value : vec) {
            value = 0.0;
        }

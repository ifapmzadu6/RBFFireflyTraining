                      ...
                      const unsigned short bits_per_sample)
{
	...
	sizeOfData = (unsigned int)(mono.size() * wavfhdr.wavfmt.blockAlign);
                      ...
                      const unsigned short samples_per_sec,const unsigned short bits_per_sample)
{
	...
	sizeOfData = (unsigned int)(stereoL.size() * wavfhdr.wavfmt.blockAlign);
    wavfhdr.sizeOfData				=	sizeOfData;
	flag = true;
}
...
void Wave::SetData(const std::vector<double> mono)
{
	...
	else
	{
		...
		monodata = mono;
		channels = 1;
		sizeOfData = (unsigned int)(monodata.size() * ( bitsPerSample / 8 ));
	}
}
...
void Wave::SetData(const std::vector<double> stereoL , const std::vector<double> stereoR )
{
	...
	else
	{
		...
		rdata = stereoR;
		channels = 2;
		sizeOfData = (unsigned int)(ldata.size() * 2 * ( bitsPerSample / 8 ));
	}
}
...
void Wave::ResamplingSinc(const unsigned short resampling,const int sincLengthHarf)
{
	...
	else
	{
		...
		else if( channels == 2 )
		{
			...
			rdata = stereoR;
			samplesPerSec = resampling;
			sizeOfData = (unsigned int)(ldata.size() * wavfhdr.wavfmt.blockAlign);
			SetHeader();
		}
	}
}